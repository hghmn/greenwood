...
const updateImportMap = (entry, entryPath) => {
  console.debug(`!!!! updateImportMap with ${entry} => ${entryPath}`);

  if (path.extname(entryPath) === '') {
    entryPath = `${entryPath}/${entry}.js`;
    console.log('massaging bare import a bit. entryPath is now...', entryPath);
  }
...
const walkPackageJson = (packageJson = {}) => {
  ...
  console.debug('walkPackageJson for', packageJson);
      ...
      if (Array.isArray(entry)) {
        ...
        exportMap.forEach((entry) => {
          ...
          if (Array.isArray(exportMapEntry)) {
            ...
            // TODO needed ? packageExport = exportMapEntry.default;
